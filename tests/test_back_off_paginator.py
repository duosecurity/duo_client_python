...
from duo_client.back_off_paginator import BackOffPaginator
...
class TestBackOffPaginator(unittest.TestCase):

    def setUp(self):
        self.client = get_duo_client()
        self.back_off_paginator = BackOffPaginator(self.client)
    ...
    def test_offset_from_md_exists(self):
        ...
        self.assertEqual(
            self.back_off_paginator._next_offset_from_metadata(metadata),
            7
        )

        self.assertEqual(self.back_off_paginator._next_offset_from_metadata(
                metadata_with_complex_offset
            ),
    ...
    def test_offset_from_md_doesnt_exist(self):
        ...
        self.assertIsNone(self.back_off_paginator._next_offset_from_metadata(
            metadata
        ))
    ...
    def test_offset_from_request_exists(self):
        ...
        self.assertEqual(
            self.back_off_paginator._next_offset_from_response(response),
            ["15329518950000123456789", "af0ba235-0b33-23c8-bc23-a31aa0231de8"]
        )

        self.assertIsNone(self.back_off_paginator._next_offset_from_response(
            response_wrong_type
        ))
    ...
    def test_next_offset_metadata_precedence(self):
        ...
        self.assertEqual(
            self.back_off_paginator._get_next_offset(response, metadata),
    ...
    @patch("duo_client.back_off_paginator.sleep")
    @patch.object(Admin, 'api_call')
    def test_throws_max_retries_exception(self, mocked_api_call, mocked_sleep):
        self.back_off_paginator = BackOffPaginator(self.client, max_attempts=3)
        ...
        with self.assertRaises(MaxRequestAttemptsReached):
            page_generator = self.back_off_paginator.request(
                "GET",
                "/adminapi/pretendapi/endpoint",
                {}
            )
            pages = [page for page in page_generator]
